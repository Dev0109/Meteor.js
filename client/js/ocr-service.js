  ...
  POST(imageData, fileName) {
    ...
    let promise = new Promise(function (resolve, reject) {
      Meteor.call("getOcrResultFromVerifi", imageData, fileName, function(error, results) {
        let getOcrResult = JSON.parse(results);
        if (error) {
          reject(error);
        } else {
          if (getOcrResult.api_request.status_code == 201) {
            resolve(getOcrResult.document.inference.prediction);
          } else {
            reject(getOcrResult.api_request.error);
          }
        }
      });
